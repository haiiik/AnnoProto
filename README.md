...
"编注成型"是一个 skill，功能为创建带标注且标注文本可编辑的 HTML 原型。输出的 HTML 是一个**自包含单文件**：内嵌标注引擎 v2（右侧抽屉式），每个可交互元素在自身右上角显示一个序号圆点，标注内容统一在页面右侧的抽屉浮窗内按页面区块分组展示。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.1 | 2026-08-03 | 结构化格式优化（字段前缀、必填是/否、去掉字段名、默认值字样）；业务逻辑标注 10 项；详情上限 300 字、抽屉 480px；项目定名 AnnoProto（编注成型） |
| v1.0.5 | 2026-08-02 | 抽屉式重构：全局分组、序号重排、标注点显隐、页面文字编辑、多页面支持、结构化格式引入 |
| v1.0.0 | — | 初始版本（气泡式标注） |
