...
创建带标注且标注文本可编辑的 HTML 原型。输出的 HTML 是一个**自包含单文件**：内嵌标注引擎 v2（右侧抽屉式），每个可交互元素在自身右上角显示一个序号圆点，标注内容统一在页面右侧的抽屉浮窗内按页面区块分组展示，双击文本即可就地编辑，点击保存按钮可一键保存（回写标注并下载）单文件 HTML。
...
2. 说明标注交互方式：
   ...
   - **双击**页面任意文字（标题、正文、按钮、表格内容等）→ 就地编辑 → 失焦/Enter 保存，Esc 取消；导出时保留修改；主标题与元素原文字相同时同步跟随新文字。
   - 点击抽屉底部"添加标注"按钮 → 点击页面元素 → 填写表单 → 生成新标注（写回 data 属性，可导出）。
   - 点击卡片右上角 × → 确认 → 删除该标注（移除 data 属性，序号重排）。
   - 点击抽屉底部"隐藏圆点"按钮可隐藏/显示页面元素序号圆点。
3. 说明保存：点击抽屉底部"保存"按钮，回写所有编辑并下载单文件 HTML（文件名加 `-annotated` 后缀）。
4. 说明收起/展开：点击抽屉底部"收起"按钮收起抽屉，页面右下角出现"展开标注"按钮可重新展开。
...
- **抽屉样式**：右侧固定浮窗，宽 480px，头部含页面名+标注数，底部固定按钮栏含添加标注/保存/标注点显隐/收起按钮；收起后页面右下角显示展开按钮。
...
**1. 生成的页面里没有标注抽屉或圆点？**
检查是否完成阶段 3 的引擎注入：页面 `<body>` 末尾必须内联 `annotation-engine.js` 与 `annotation-engine.css`，且原型存在至少一个 `data-annotate` 元素。运行 `validate_prototype.py` 校验可定位缺失项。

**2. 双击页面文字没反应？**
双击的目标必须是页面正文区域内的元素（含文字内容），且不能是 `input`/`textarea`/`select`。引擎自身的抽屉、浮层、展开按钮区域不支持页面文字编辑。

**3. 多页面原型如何切换标注？**
为每个页面容器加 `data-page`（可选 `data-page-title`），原型导航切换页面显示状态时引擎自动监听并刷新抽屉为当前页标注。也可在导航按钮上调用 `window.__annoGotoPage(pageId)`。

**4. 点"保存"后文件下载到哪里？**
浏览器按下载设置保存，文件名为原文件名加 `-annotated` 后缀（如 `login-annotated.html`），是自包含单文件，重新打开标注仍在且可继续编辑。

**5. 结构化格式怎么填？**
表单字段：`类型：值 | 规则/交互逻辑：值 | 必填：是/否 | 默认值：值`；列表数据列：`类型：值 | 数据来源：值 | 说明：值`。字段名称由主标题（`data-annotate`）承载，不写入详情；业务逻辑用 `当[条件]时 → [行为]；否则[兜底]` 三段式 + `前置条件：`/`异常：`/`权限：`/`流转：` 前缀段落。

**6. 序号为什么与页面 DOM 顺序不一致？**
序号按抽屉显示顺序重新编排（分组顺序 + 组内顺序），保证抽屉内永远从小到大；页面圆点与卡片序号一致，与 DOM 顺序无关。

**7. 怎么隐藏页面元素上的序号圆点？**
点击抽屉底部"隐藏圆点"按钮可隐藏/显示全部圆点。

**8. 如何给原型手动添加标注？**
点击抽屉底部"添加标注"按钮 → 进入拾取模式（元素 hover 高亮）→ 点击目标元素 → 表单中填写主标题/类型/详情/区块 → 保存。新标注写回 `data-annotate` 属性，随"保存"导出。

**9. 如何删除某条标注？**
将鼠标移到抽屉内对应卡片，卡片右上角出现 ×，点击后确认删除。删除会移除该元素的 `data-annotate` 相关属性、圆点与卡片，序号自动重排。

**10. 修改了页面元素文字，标注会跟着变吗？**
仅当标注主标题与元素原文字完全相同时才会同步跟随新文字（如主标题"登录"、元素文字"登录"，把文字改成"提交登录"后主标题自动更新）；业务命名的主标题（如"提交按钮"）不会被覆盖。
