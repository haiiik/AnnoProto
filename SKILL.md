...
- **标注文案**：主标题 ≤ 12 字，名词性短语（表单/数据类取字段名称）；详情 ≤ 300 字，操作类描述"做什么 + 触发什么"，表单字段/数据列用结构化格式（表单字段：`类型：值|规则/交互逻辑：值|必填：必填/选填|默认值：值`；数据列：`类型：值|数据来源|说明`，字段名称由主标题承载不重复，各字段带"字段名："前缀），业务逻辑用 `当[条件]时 → [行为]；否则[兜底]` 三段式 + 前置条件/异常/权限/流转前缀段落；区块名 ≤ 20 字，名词性短语。
...
- **抽屉样式**：右侧固定浮窗，宽 480px，标题栏含页面名+标注数+保存/标注点显隐/收起按钮；收起后页面右下角显示展开按钮。
